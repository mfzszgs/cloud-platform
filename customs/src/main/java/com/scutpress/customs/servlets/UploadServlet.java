...
public class UploadServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
			...
			while (i.hasNext()) {
				...
				if (!fi.isFormField()) {// 如果是文件,进行上传操作
					...
					if (fi.getName() != null && !fi.getName().equals("")) {
						...
						newName = userId + "_" + dateToString(new Date()) + ".pdf";// new
																					// Date()为获取当前系统时间

						// File file = new File(path, tempFile.getName());
						// //上传文件的保存路径 在这里要把文件换个名字
		...
		/*
		 * HttpSession session = request.getSession(); String title =
		 * request.getParameter("docTitle"); System.out.println("docTitle"+
		 * title); int classId =
		 * Integer.parseInt(request.getParameter("selectDocClass")); String
		 * loginName =
		 * (String)session.getAttribute("loginName");//通过loginName找userId
		 * 
		 * response.setContentType("text/html;charset=UTF-8");
		 * request.setCharacterEncoding("UTF-8");
		 * 
		 * try { DAO dao = new DAO(); boolean flag = dao.addDocOne(title,
		 * classId, loginName); session.setAttribute("addDocOneResult",flag);
		 * response.sendRedirect("UploaderResult.jsp"); } catch (SQLException e)
		 * { e.printStackTrace(); }
		 */
	}
