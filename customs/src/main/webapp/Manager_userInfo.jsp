...
<body>
	...
	<%
		...
		List<UsersBean> li = managedao.getProcesserUsers();
		...
		//	e.printStackTrace();
		//}
		int resultconts = li.size();
		int totalPages = m_pages.getTotal_Pages(resultconts);//取出总页数
		Iterator<UsersBean> it = li.iterator();
		Iterator<UsersBean> itet = m_pages.getPageSet(it, curPages);
	%>
	...
	<form method="post" name="form3">
		<div class="M_table">
			<p>
				...
				<a
					href="LogoutServlet?loginName=<%=session.getAttribute("loginName")%>">注销</a>
			</p>
			<table width="832" border="1" align="center" cellpadding="1"
				...
				<tr bgcolor="#FFFFFF">
					...
					<%
						while (itet.hasNext()) {
							UsersBean bean = itet.next();
					%>
					<td><%=bean.getLoginName()%></td>
					<td height="33"><%=bean.getName()%></td>
					<td><%=dao.getUserClass(bean.getUserClass()).get(0)%></td>
					<td style="width: 38px;"><a
						href="Manager_userInfo_detail.jsp?id=<%=bean.getId()%>"> <img
							src="images/change.png" alt="编辑" width="25" height="25"
							hspace="10">
					</a></td>
					<td><a
						href="DelServlet?id=<%=bean.getId()%>&jsp=Manager_userInfo"> <img
							src="images/dele.png" width="24" height="24" hspace="10">
					</a></td>
				</tr>
