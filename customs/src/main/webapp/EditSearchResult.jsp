...
<html>
  <head>
	...
	/* 执行操作 */
	function do_action_preview() {
		if (!checkAll("form", "choose")) {
			alert("没有checkbox被选中，提示用户至少选择一个！");
		} else if (checkAll("form", "choose")) {
			document.form.action="EditPreview.jsp";
			document.form.submit();
		}
	}
	
	function do_action_formdoc() {
		if (!checkAll("form", "choose")) {
			alert("没有checkbox被选中，提示用户至少选择一个！");
		} else if (checkAll("form", "choose")) {
			document.form.action="EditFormDoc.jsp";
			document.form.submit();
		}
	}
</script>
  ...
  <body>
			...
			<table width="960" border="1" align="center" cellpadding="1"
				...
				<tr >
					...
					<td width="49">
						<div>
							<span class="STYLE19"> 
								<input type="checkbox" name="alls" onClick="selectAll('form','choose')" value="全选">
							</span>
						</div>
					</td>
				</tr>
    		...
    		docb = (DocBean) aIter.next();
    	  	...
    	  	<tr>
				<td ><%=docb.getTitle()%></td>    	  	
    	  		...
    	  		<td style="width:38px;"> <%=docb.getBuilder()%></td> 
    	  		<td style="width:38px;">
    	  		<input type="checkbox" name="choose" value="<%=docb.getID()%>">
				</td> 
   	  	    </tr>
    	<%  	
    	 }
    %>
		...
		<input type="button" name="preview" value="预览" onclick="do_action_preview();">
		<input type="button" name="formdoc" value="生成文档集" onclick="do_action_formdoc();">
